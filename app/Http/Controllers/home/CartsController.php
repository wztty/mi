...
class CartsController extends Controller
{
     ...
     * sku-id对应的键值是一个数组 代表sku-id
     * sku-id对应的键值是一个数组 代表购物车商品数量
     * sku-id对应的键值是一个数组 代表商品的good-id
     */
    //初始化数据
    ...
    public function getAddproduct(Request $request)
    {
        ...
        if(empty($uid)){
            ...
            $request->session()->push('cart.'.($sku->id), $sku->id);//代表 id
            $request->session()->push('cart.'.($sku->id), $request->input('num'));//代表 num
            $request->session()->push('cart.'.($sku->id), $sku->good_id);//代表 gid good_id
            ...
            if($skus&&in_array($request->input('id'),$skus)){
                ...
                //未全选状态处理
            }else if(empty($request->input('listItem'))){
...
//            dd(1);
            ...
            if(empty($cart)){
                ...
                $cart->goods_num = $cart->goods_num + (int)($sku->$num);
